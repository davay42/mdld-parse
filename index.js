...
function render(src, options = {}) {
	...
	function getAttrs(rawAnnotation, parsedSem, isLink = false, fallbackHref = null) {
		const attrs = [];
		const classes = [];
		if (rawAnnotation) attrs.push(`data-annotation="${escapeHtml$1("{" + rawAnnotation + "}")}"`);
		let iri = null;
	...
	function renderInline(text) {
		...
		if (carriers.length === 0) return escapeHtml$1(text);
		let out = "";
		let lastPos = 0;
		for (const c of carriers) {
			if (c.range[0] > lastPos) out += escapeHtml$1(text.slice(lastPos, c.range[0]));
		...
		if (lastPos < text.length) out += escapeHtml$1(text.slice(lastPos));
		return out;
	}
	const out = [];
	let i = 0;
	...
	while (i < lines.length) {
		const line = lines[i];
		if (codeBlock) {
			if (new RegExp(`^${codeBlock.fenceChar}{${codeBlock.fenceLength},}\\s*$`).test(line.trim())) {
				...
				out.push(`<pre><code ${attrString}>${escapeHtml$1(carrierText)}</code></pre>`);
				codeBlock = null;
			} else codeBlock.content.push(line);
			i++;
			continue;
		}
		if (line.trim() === "") {
		...
		if (prefix) {
			...
			out.push(`<div class="mdld-prefix" data-prefix="${prefix.prefix}" data-raw="${escapeHtml$1(prefix.iri)}" data-iri="${escapeHtml$1(resolvedIri)}" style="display:none"></div>`);
			i++;
			continue;
		}
		...
		if (heading) {
			...
			const attrString = mergeClasses("mdld-heading", getAttrs(heading.attrs, sem));
		...
		if (detectList(line)) {
			...
			while (i < lines.length) {
				...
				const attrString = mergeClasses("mdld-item", getAttrs(l.attrs, sem));
		...
		if (blockquote) {
			...
			const attrString = mergeClasses("mdld-quote", getAttrs(blockquote.attrs, sem));
		...
		if (standalone) {
			...
			const attrString = mergeClasses("mdld-standalone", getAttrs(`=${standalone.content}`, fakeSem));
		...
		let paraLines = [line];
		i++;
		while (i < lines.length && lines[i].trim() !== "" && !detectFence(lines[i].trim()) && !detectPrefix(lines[i]) && !detectHeading(lines[i]) && !detectList(lines[i]) && !detectBlockquote(lines[i]) && !detectStandaloneSubject(lines[i])) {
...
function deconstruct(html) {
	...
	while (pos < html.length) {
		...
		if (html.startsWith("<div class=\"mdld-prefix\"", pos)) {
			...
			const end = html.indexOf("</div>", pos);
			if (end === -1) break;
			const ann = extractAttr(html.slice(pos, end + 6), "data-annotation");
			...
			const preClose = full.indexOf(">") + 1;
			const codeOpenEnd = full.indexOf(">", preClose) + 1;
			const codeTag = full.slice(preClose, codeOpenEnd);
			const content = full.slice(codeOpenEnd, full.length - 13);
			const langMatch = (extractAttr(codeTag, "class") || "").match(/language-(\w+)/);
			const lang = langMatch ? langMatch[1] : "";
			...
			const ann = extractAttr(openTag, "data-annotation");
			const quoted = deconstructInline(content).split("\n").map((l) => l.trim() ? `> ${l}` : ">").join("\n");
...
function deconstructInlineElement(tagName, attrs, inner) {
	...
	switch (tagName) {
		case "a": return `[${content}](${extractAttr(attrs, "href") || ""})${suffix}`;
...
function extractAttr(tag, name) {
	...
	if (!m) return null;
	return unescapeHtml(m[1] !== void 0 ? m[1] : m[2]);
}
...
//#endregion
export { DEFAULT_CONTEXT, DataFactory, deconstruct, expandIRI, generate, generateNode, getIRIColor, hash, hashIRI, highlightMDLD, locate, merge, parse, parseSemanticBlock, render, shortenIRI, updateValue };
