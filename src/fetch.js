...
const ACCEPT =
  "text/markdown, text/x-mdld, text/plain;q=0.9, text/html;q=0.8, application/xhtml+xml;q=0.8, */*;q=0.5";
const DEFAULT_MAX_DEPTH = 5;
const DEFAULT_CONCURRENCY = 5;
const DEFAULT_MAX_PAGES = 100;
const DEFAULT_ACCEPT_RE = /\.(md|mdld|markdown|html?|xhtml)$/i;
const BINARY_CT = /image\/|audio\/|video\/|font\/|application\/(octet-stream|zip|gzip|pdf|wasm|protobuf)/i;
...
function normalizeUrl(raw, base) {
  ...
  try {
    ...
    if (!HTTP_PROTOCOLS.has(url.protocol)) return null; // no file:, javascript:, data:, mailto:
    url.hash = "";
    return url.href;
  } catch {
    return null;
...
function defaultAccept(url) {
  ...
  if (!file.includes(".")) return true; // SSR routes, clean URLs
...
// ════════════════════════ Link extraction ═══════════════════════════
//
// Public entry: run BOTH extractors on any text. CommonMark allows inline
// HTML inside markdown, so markdown documents can carry <a href> links and
// HTML documents can carry markdown link syntax — scanning for both is the
// only correct posture for mixed content.
...
function findLinkEnd(line, start) {
  if (line[start] === "<") {
    ...
    return end === -1 ? null : { urlStart: start + 1, urlEnd: gt, end };
  }
  let depth = 0;
  let i = start;
  while (i < line.length) {
    ...
    if (c === "(") depth++;
      ...
      if (depth === 0) return { urlStart: start, urlEnd: i, end: i };
      depth--;
    } else if (c === " " || c === "\t") {
      const end = findTitle(line, i);
      if (end === -1) return null;
      return { urlStart: start, urlEnd: i, end };
    }
    i++;
  }
  return null;
...
function extractHTMLLinks(text, base, out) {
  ...
  while (i < text.length) {
    ...
    const t1 = text[i + 1];
    // Raw-text elements: skip contents (JS/CSS strings are link-shaped noise)
    if (t1 === "s" || t1 === "S") {
      const name = rawTagName(text, i);
      if (name) { i = skipRawElement(text, i, name); continue; }
    }
...
function rawTagName(text, i) {
  // text[i] === '<'
  const a = text[i + 1] === "s" || text[i + 1] === "S" ? text[i + 1] : null;
  if (!a) return null;
  const rest4 = text.slice(i + 2, i + 7).toLowerCase(); // after 's'
  if (rest4.startsWith("cript")) {
    const after = text[i + 7];
    if (isNameBoundary(after) || after === undefined) return "script";
  }
  if (rest4.startsWith("tyle")) {
    const after = text[i + 6];
    if (isNameBoundary(after) || after === undefined) return "style";
  }
  return null;
}
...
function skipRawElement(text, start, name) {
  ...
  while (i < text.length) {
    ...
    if (text.slice(c + 2, c + 2 + name.length).toLowerCase() === name) {
      ...
      if (isNameBoundary(after) || after === undefined) {
...
export async function crawl(startUrl, options = {}) {
  ...
  if (typeof fetchFn !== "function")
    throw new TypeError("crawl: no fetch implementation available — pass options.fetchFn");

  const start = normalizeUrl(startUrl, baseHref);
  if (!start)
    throw new TypeError(
      `crawl: cannot resolve "${startUrl}" to an absolute http(s) URL (baseHref: ${baseHref ?? "none"})`
    );

  if (signal?.aborted) return { urls: [], pages: [], errors: [], aborted: true };

  const startOrigin = new URL(start).origin;

  // Soft-fail wrapper: broken cache degrades to "no cache", never to failure.
  const store = {
    get: async (url) => { try { return (await cache.get(url)) ?? null; } catch { return null; } },
    set: async (url, entry) => { try { await cache.set(url, entry); } catch { /* ignore */ } },
  };
  ...
  const isAllowed = (url) =>
    (!sameOrigin || new URL(url).origin === startOrigin) && accept(url);
  ...
  async function process(job) {
    ...
    try {
      const res = await fetchFn(url, {
        ...
        redirect: "follow",
        cache: "no-store", // this module's cache is the single authority
      });
      status = res.status;
      ...
      if (res.status === 304 && entry) {
        ...
        await store.set(url, entry); // touch LRU recency
      } else if (res.ok) {
        contentType = res.headers.get("content-type") || "";
        if (BINARY_CT.test(contentType)) {
          try { await res.body?.cancel(); } catch { /* ignore */ }
          errors.push({ url, kind: "binary", status, message: `unsupported content-type: ${contentType}` });
          return;
        }
        text = await res.text();
        await store.set(url, {
          text, contentType, finalUrl,
          etag: res.headers.get("etag"),
          lastModified: res.headers.get("last-modified"),
          timestamp: Date.now(),
        });
      } else if (res.status >= 500 && entry) {
        text = entry.text; // stale-if-error
        contentType = entry.contentType || "";
        finalUrl = entry.finalUrl || finalUrl;
        stale = true;
      } else {
        errors.push({ url, kind: "http", status: res.status, message: `HTTP ${res.status} for ${url}` });
        return;
      }
    } catch (err) {
      if (signal?.aborted || err?.name === "AbortError") { aborted = true; return; }
      if (entry) {
        text = entry.text; // stale-if-error on network failure
        contentType = entry.contentType || "";
        finalUrl = entry.finalUrl || url;
        stale = true;
      } else {
        errors.push({ url, kind: "network", message: String(err?.message || err) });
        return;
      }
    }

    // Synchronous section: extraction + visited check-and-add happen with no
    // await in between, so concurrent workers can never double-enqueue.
    const links = new Set();
    extractMarkdownLinks(text, finalUrl, links);
    extractHTMLLinks(text, finalUrl, links);

    const page = {
      url, finalUrl, depth, status,
      contentType,
      kind: looksLikeHTML(contentType, text) ? "html" : "text",
      stale,
      text,
      links: [...links], // full normalized outgoing edge list (pre accept-filter)
    };
    pages.set(url, page);
    ...
    try {
      ...
      errors.push({ url, kind: "callback", message: String(err?.message || err) });
    }
  }

  // Event-driven pump: always keeps `concurrency` fetches in flight; newly
  // discovered links start the moment a slot frees (no batch head-of-line).
  ...
  for (; ;) {
    while (cursor < queue.length && inflight.size < concurrency && !aborted) {
      ...
      const p = process(job)
        .catch((err) => {
          errors.push({ url: job.url, kind: "internal", message: String(err?.message || err) });
        })
  ...
  // Deterministic: queue holds every enqueue in BFS discovery order.
  const ordered = [];
